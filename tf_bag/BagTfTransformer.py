...
class BagTfTransformer(object):
    ...
    def __init__(self, bag):
        ...
        if IS_ROS2:
            self.tf_times = np.array(
                list(
                    (
                        rclpy.time.Time.from_msg(tfm.header.stamp).nanoseconds
                        for tfm in self.tf_messages
                    )
                )
            )
        else:
    ...
    def _read_bag(bag):
        if IS_ROS2:
            ...
            while reader.has_next():
                ...
                transforms = [
                    BagTfTransformer._remove_slash_from_frames(tm)
                    for tm in msg.transforms
                ]
            ...
            tf_messages.sort(
                key=lambda tfm: rclpy.time.Time.from_msg(tfm.header.stamp).nanoseconds
            )
            tf_static_messages.sort(
                key=lambda tfm: rclpy.time.Time.from_msg(tfm.header.stamp).nanoseconds
            )
        else:
            if type(bag) == str:
    ...
    def _time_from_tf_msg(msg):
        if IS_ROS2:
            return rclpy.time.Time.from_msg(
                msg.header.stamp, clock_type=rclpy.clock.ClockType.SYSTEM_TIME
            )
        else:
    ...
    def getMessagesInTimeRange(self, min_time=None, max_time=None):
        ...
        indices_in_range = np.where(
            np.logical_and(min_time < self.tf_times, self.tf_times < max_time)
        )
    ...
    def populateTransformerAtTime(self, target_time, buffer_length=10, lookahead=0.1):
        ...
        """
        if IS_ROS2:
            target_start_time = target_time - rclpy.time.Duration(
                seconds=min(
                    min(buffer_length, 10) - lookahead,
                    target_time.nanoseconds / 1000000000,
                )
            )  # max buffer length of tf Transformer
            target_end_time = target_time + rclpy.time.Duration(
                seconds=lookahead
            )  # lookahead is there for numerical stability
        else:
            target_start_time = target_time - rospy.Duration(
                min(min(buffer_length, 10) - lookahead, target_time.to_sec())
            )  # max buffer length of tf Transformer
            target_end_time = target_time + rospy.Duration(
                lookahead
            )  # lookahead is there for numerical stability
        ...
        tf_messages_in_interval = self.getMessagesInTimeRange(
            population_start_time, target_end_time
        )
    ...
    def _filterMessages(
        ...
        if IS_ROS2:
            if isinstance(start_time, builtin_interfaces.msg.Time):
                start_time = rclpy.time.Time.from_msg(
                    start_time, clock_type=rclpy.clock.ClockType.SYSTEM_TIME
                )
            if isinstance(end_time, builtin_interfaces.msg.Time):
                end_time = rclpy.time.Time.from_msg(
                    end_time, clock_type=rclpy.clock.ClockType.SYSTEM_TIME
                )
        ...
        if start_time:
            messages = filter(
                lambda m: BagTfTransformer._time_from_tf_msg(m) > start_time, messages
            )
        if end_time:
            messages = filter(
                lambda m: BagTfTransformer._time_from_tf_msg(m) < end_time, messages
            )
        return messages

    def getTransformMessagesWithFrame(
        self, frame, start_time=None, end_time=None, reverse=False
    ):
        """
    ...
    def getTransformFrameTuples(self):
        ...
        if self.all_transform_tuples is None:
            ...
            self.static_transform_tuples = {
                (m.header.frame_id, m.child_frame_id) for m in self.tf_static_messages
            }
    ...
    def getAverageUpdateFrequency(
        self, orig_frame, dest_frame, start_time=None, end_time=None
    ):
        """
    ...
    def getTransformUpdateTimes(
        ...
        """
        trigger_frames_were_provided = (
            trigger_orig_frame is not None or trigger_dest_frame is not None
        )
        ...
        first_update_time = self.waitForTransform(
            orig_frame, dest_frame, start_time=start_time
        )
        return (
            t
            for t in BagTfTransformer._getTimeFromTransforms(updates)
            if t > first_update_time
        )
    ...
    def waitForTransform(self, orig_frame, dest_frame, start_time=None):
        ...
        if start_time is not None:
            ...
            messages = filter(
                lambda m: BagTfTransformer._time_from_tf_msg(m) > start_time,
                self.tf_messages,
            )
        else:
            messages = self.tf_messages
        missing_transforms = (
            set(self.getChainTuples(orig_frame, dest_frame))
            - self.static_transform_tuples
        )
        ...
        try:
            ...
            raise ValueError(
                "Transform not found between {} and {}".format(orig_frame, dest_frame)
            )
        return ret
    ...
    def lookupTransform(self, orig_frame, dest_frame, time):
        ...
        self.populateTransformerAtTime(time)
        try:
            common_time = self.transformer.get_latest_common_time(
                orig_frame, dest_frame
            )
        except:
            raise RuntimeError(
        ...
        return self.transformer.lookup_transform_core(
            orig_frame, dest_frame, common_time
        )
        ...
        ret = (
            (
                ...
                self.lookupTransform(
                    orig_frame=orig_frame, dest_frame=dest_frame, time=t
                ),
            )
    ...
    def getFrameAncestors(self, frame, early_stop_frame=None):
        ...
        frame_chain = [frame]
        chain_link = list(
            filter(lambda tt: tt[1] == frame, self.getTransformFrameTuples())
        )
        while chain_link and frame_chain[-1] != early_stop_frame:
            frame_chain.append(chain_link[0][0])
            chain_link = list(
                filter(
                    lambda tt: tt[1] == frame_chain[-1], self.getTransformFrameTuples()
                )
            )
    ...
    def getChain(self, orig_frame, dest_frame):
        ...
        while len(dest_ancestors) > 0 and orig_ancestors[0] == dest_ancestors[0]:
            if (
                len(orig_ancestors) > 1
                and len(dest_ancestors) > 1
                and orig_ancestors[1] == dest_ancestors[1]
            ):
    ...
    def averageTransforms(transforms):
        ...
        mean_quaternion = quaternions.mean(
            axis=0
        )  # I know, it is horrible.. but for small rotations shouldn't matter
    ...
    def averageTransformOverTime(
        ...
        target_transforms = (
            self.lookupTransform(orig_frame=orig_frame, dest_frame=dest_frame, time=t)
            for t in update_times
        )
        return self.averageTransforms(target_transforms)

    def replicateTransformOverTime(
        self, transf, orig_frame, dest_frame, frequency, start_time=None, end_time=None
    ):
        """
        ...
        t_msg = TransformStamped(
            ...
            transform=Transform(
                translation=Vector3(*transl), rotation=Quaternion(*quat)
            ),
        )
        ...
        new_msgs = [
            createMsg(t)
            for t in range(
                start_time.to_nsec(), end_time.to_nsec(), time_delta.to_nsec()
            )
        ]
